    ...
    function doRequest() {
        wx.request(utils.extend({}, options, {
            ...
            success: function (response) {
                ...
                // 如果响应的数据里面包含 SDK MagicID，表示被服务端 SDK 处理过，此时一定包含登录态无效的信息
                if (data && data[constants.WX_SESSION_MAGIC_ID]) {
                    // 清除登录态
                    Session.clear();

                    var error, message;
                    if (data.error == constants.ERR_SESSION_EXPIRED) {
                        // 如果登陆过期，并且还没重试过，会尝试登录后刷新凭据重新请求
                        if (!hasRetried) {
                            hasRetried = true;
                            doRequestWithLogin();
                            return;
                        ...
                        message = '鉴权服务器检查登录态发生错误(' + data.error + ')：' + (data.message || '未知错误');
